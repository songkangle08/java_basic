/*
  基本数据类型与String的运算

  一： 关于String的理解
    1、String类，属于引用数据类型
    2、String类型的变量，可以使用一对双引号""的方式进行赋值
    3、String声明的字符串内部，可以包含0个，1个或多个。
  
  二：String与基本数据类型变量间的运算
    1、这的基本数据类型包括boolean在内的8种
    2、String与基本数据类型变量间只能做连接运算，使用+表示
    3、运算的结果只能是String类型
    
*/ 

public StringTest{
  public static void main(String[] args){
    String str1 = "Hello World";

    System.out.println("str1");
    System.out.println(str1);

    String str2 = "";

    String str3 = "a";  // char c1 = 'a';  // 数据类型不一样


    int num1 = 10;
    boolean b1 = true;
    String str5 = "hello";

    System.out.println(str5+b1);  // hellotrue


    String str6 = str5+b1+num1;
    System.out.println(str6); // hellotrue10

    // 思考：如下的声明编译能通过?
    // String str7 = b1 + num1 + str4;  // 不能编译

    // 如何将String类型的变量转换为基本数据类型？
    int num2 = 10;
    String str7 = num2 + ""; // "10";
    String str8 = "abc";  // 不能考虑转换为基本数据类型

    // 编译不通过
    // int num3 = (int)str8;  


    // 如何实现呢？ 使用Integer
    int num3 = Integer.parsetInt(str8);
    System.out.println(num3);

  }
} 